...
class VAE(nn.Module):
    ...
    def forward(self, x_in):
        ...
        z, eps = self.sample(z_mean, z_logvar)
        x_out = self.decode(z)
        return x_out, z_mean, z_logvar, eps, z

def logpz(z):
    return -torch.sum(0.5*(z ** 2 + np.log(2 * math.pi)))

def logqz_x(log_var, eps):
    return -torch.sum(0.5 * (eps ** 2 + np.log(2 * math.pi) +  log_var * 0.5))

def logpx_z(x_out, x_in):
    return -F.binary_cross_entropy(x_out,x_in, size_average=False)
# Loss function
def criterion(x_out, x_in, z_mu, z_logvar, eps, z):
    # ELBO = -DK(q(z|x)|| p(z)) + logp_theta(x|z)
    #      = 1/2(1 + log(var) - mu ^2 - var) +  logp_theta(x|z)
    logqzx = logqz_x(z_logvar, eps)
    logpz_ = logpz(z)
    logpxz = logpx_z(x_out, x_in)
    loss = -(logpxz + logpz_ - logqzx) / x_out.size(0)
    return loss
...
def train(model,optimizer,dataloader,epochs=15):
    ...
    for epoch in range(epochs):
        for images,_ in dataloader:
            x_in = (images)
            optimizer.zero_grad()
            x_out, z_mu, z_logvar, eps, z = model(x_in)
            loss = criterion(x_out,x_in,z_mu,z_logvar, eps, z)
            loss.backward()
            optimizer.step()
