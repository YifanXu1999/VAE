...
class VAE_NF(nn.Module):
    ...
    def forward(self, x):
        ...
        logpz = -torch.sum(0.5*(z ** 2 + np.log(2 * math.pi))) / x.size(0)
        logqz_x = -torch.sum(0.5 * (eps ** 2 + np.log(2 * math.pi) +  logvar * 0.5))
        w, u, b = self.getw_u_b(x)
        z, sum_ladj = self.nf(z, w, u, b)
        logqz_x = logqz_x / x.size(0) - sum_ladj.mean()
        x_out = self.decode(z)
        return x_out, logqz_x, logpz


def criterion(x_out, x_in, logqz_x, logpz):
    # ELBO = -DK(q(z|x)|| p(z)) + logp_theta(x|z)
    #      = 1/2(1 + log(var) - mu ^2 - var) +  logp_theta(x|z)
    logpx_z = -F.binary_cross_entropy(x_out,x_in,size_average=False) / x_out.size(0)
...
def train(model,optimizer,dataloader,epochs=15):
    ...
    for epoch in range(epochs):
        ...
        for images,_ in dataloader:
            ...
            optimizer.zero_grad()
            x_out, logqz_x, logpz= model(x_in)
            loss = criterion(x_out,x_in,logqz_x, logpz )
            loss.backward()
            optimizer.step()
